...
<p>Small fixes by Michael Thies, 2015</p>
...
signaled as ctrl-E at the beginning of a new line. When reading from a file,
...
private static LinkedList<InputStreamReader> inputStack;
...
/** Read a raw Unicode character.
If an attempt is made to read beyond the end of the file,
eof is returned and done() yields false. Otherwise the read char
is a normal Unicode character in the full frange 0..65535.
*/
...
/** Read a character, but skip white spaces.
If an attempt is made to read beyond the end of the file,
eof is returned and done() yields false. Otherwise the read char
is a normal Unicode character in the full frange 0..65535.
*/
...
public static boolean readBoolean() {
  ...
  else { done = s.equalsIgnoreCase("false"); return false; }
}
...
public static void open(String fn) {
  try {
    InputStreamReader s = new FileReader(fn);
    bufferStack.add(buf);
    inputStack.add(in);
    in = s;
    done = true;
